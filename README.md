...
Splatorium は、画像から 3D Gaussian Splat を生成し、ブラウザ上の倉庫とシーンで整理・配置できるローカル 3D ワークベンチです。生成処理には ComfyUI と TripoSplat を使用します。保存したモデルやシーンは、同じ LAN 上の端末から利用できます。

![生成したモデルを配置するシーン編集画面](docs/onesheet/shots/d-scene.png)
...
| 画像を選んで生成 | 完成したモデルを倉庫で確認 |
|---|---|
| ![生成ダイアログ](docs/onesheet/shots/a-generate.png) | ![モデルとシーンの倉庫](docs/onesheet/shots/b-warehouse.png) |
